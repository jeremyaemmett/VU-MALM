...
def read_site_data(site, model_depths):
    ...
    site_o2 = forcing.read_o2(params.site, site_comp, model_depths)
    site_doc = forcing.read_doc(params.site, site_comp, model_depths)
    site_genes = forcing.read_genes(params.site, site_comp, model_depths)
    ...
    # For each campaign, compute a leading factor (f_doc) for the purpose of dynamically calculating DOC
    # (DOC = f_doc * v-metric h2o * v-metric soc * temp)...
    # ... then average f_doc over all campaigns
#    months = ['jun','aug']
#    f_docs = []
#    for month in months:
#        f_docs.append(np.mean(site_doc[site + '_' + month + '_doc'] / (site_comp[site + '_' + month + '_h2o_m3_m3'] *
#                                                             site_temps[site + '_' + month + '_mean_temp_depth'] *
#                                                             site_comp[site + '_' + month + '_soc_m3_m3'])))
#
    #f_doc = np.mean(f_docs)

    return(site_roots, site_comp, site_temps, site_o2, site_doc, site_genes, site_albedo, site_fluxes, site_geo)
